$(function () {
    var l = abp.localization.getResource('acme.student');
    ...
    var dataTable = $('#LopHocTable').DataTable(abp.libs.datatables.normalizeConfiguration({
        processing: true,
        serverSide: true,
        paging: true,
        searching: false,
        ...
        scrollCollapse: true,
        ordering: false,
        ajax: abp.libs.datatables.createAjax(acme.student.commons.lopHoc.getList),
        columnDefs: [
            {
                render: function (data, type, full, meta) {
                    var table = $('#LopHocTable').DataTable();
            ...
            { data: "ghiChu", width: "20%" },
            {
                ...
                data: "id", render: function (data) {
                    var htmlRender = '';
                    if (abp.auth.isGranted('Common.LopHoc.Update')) {
                    ...
                    if (abp.auth.isGranted('Common.LopHoc.Delete')) {
        ...
        "fnDrawCallback": function (oSettings) {
            if ($('#LopHocTable').DataTable().page.info().pages < 2) {
    ...
    $('#LopHocTable tbody').on('click', 'button', function () {
    ...
    function Search() {
        ...
        dataTable = $('#LopHocTable').DataTable(abp.libs.datatables.normalizeConfiguration({
            ...
            scrollCollapse: true,
            ordering: false,
            ajax: abp.libs.datatables.createAjax(acme.student.commons.lopHoc.search, function () {
                return input;
            }),
            columnDefs: [
                {
