$(function () {
    ...
    var dataTable = $('#sinhVienTable').DataTable(abp.libs.datatables.normalizeConfiguration({
        processing: true,
        serverSide: true,
        paging: true,
        searching: false,
        ...
        scrollCollapse: true,
        ordering: false,
        ajax: abp.libs.datatables.createAjax(acme.student.commons.sinhVien.getList),
        ...
        columnDefs: [
            ...
            { data: "tuoi", width: "20%" },
            { data: "cmnd", width: "20%" },
            { data: "lopHoc", width: "20%" },
            {
                className: "action_table",
                data: "id", render: function (data) {
                    var htmlRender = '';
                    if (abp.auth.isGranted('Common.SinhVien.Update')) {
                    ...
                    if (abp.auth.isGranted('Common.SinhVien.Delete')) {
        ...
        "fnDrawCallback": function (oSettings) {
            if ($('#SinhVienTable').DataTable().page.info().pages < 2) {
    ...
    $('#SinhVienTable tbody').on('click', 'button', function () {
    ...
    function Search() {
        ...
        dataTable = $('#SinhVienTable').DataTable(abp.libs.datatables.normalizeConfiguration({
            processing: true,
            serverSide: true,
            paging: true,
            searching: false,
